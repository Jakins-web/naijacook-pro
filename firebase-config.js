...
if (!DEMO_MODE) {
  ...
  authReady = new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error('Sign-in timed out — check that Anonymous auth is enabled in Firebase Console')), 8000);
    onAuthStateChanged(auth, (user) => { if (user) { clearTimeout(timeout); resolve(user); } });
    signInAnonymously(auth).catch((err) => { clearTimeout(timeout); reject(err); });
  });
}
